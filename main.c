...
void	pb(t_list *stack_a, t_list *stack_b)
{
	...
	prev_a_pb = last_a->prev;
	last_b->next = last_a;
	prev_a_pb->next = NULL;
	prev_a_pb= last_b->next;
	last_a->prev = last_b;
	prev_a_pb->next = NULL;
}
...
int main(int argc, char **argv)
{
	...
	if (argc <=5)
		write(1, "small\n", 6);
		int twotimes = 0;
	while(numbers_in_order(stack_a))
	{
		// makes LIS
		outer = stack_a;
		printf("start loop stack a %d stack b %d", stack_a->num, stack_b->num);
		while(outer)
		{
			inner = stack_a;
		...
		//showing whole sequence that need to be pushed to stack_b
		t_list	*longest_temp = longest;
		while(longest_temp)
		{
			printf("subs number %d\n", longest_temp->num);
			longest_temp = longest_temp->subs;
		}
		...
		while(last && last->num)
		{
			//printf("ptr last = %p  last->num = %d and subs%p\n", last, last->num, last->subs);
			//printf("last->num = %d \n", last->num);
			temp = last;
			last = last->prev;
			if (temp == longest)
		...
		while(temp_a)
		{
			printf("while stack_a = %d\n", temp_a->num);
			temp_a = temp_a->next;
		}
		while(temp_b)
		{
			printf("while stack b = %d\n", temp_b->num);
			temp_b = temp_b->next;
		}
		return (0);
		if (twotimes == 1)
